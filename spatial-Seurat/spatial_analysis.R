...
# See https://github.com/satijalab/seurat-data
...
# brain already has identified gene symbols in
# brainx@assays$Spatial@data@Dimnames[[1]]

## Data preprocessing-------------
plot1 <- VlnPlot(brain, features = "nCount_Spatial", pt.size = 0.1) + NoLegend()
plot2 <- SpatialFeaturePlot(
  brain, features = "nCount_Spatial") + 
...
png(filename = "spatial_vignette_ttr.png", height = 700, width = 1200)
print(plot)
dev.off()

### 3. plot in Rstudio with different parameters
p1 <- SpatialFeaturePlot(
  brain, features = "Ttr", pt.size.factor = 1)
p2 <- SpatialFeaturePlot(
  brain, features = "Ttr", alpha = c(0.1, 1))
p1 + p2

jpeg(filename = "spatial_vignette_ttr2.jpg", height = 700, width = 1200, quality = 50)
print(p1 + p2)
dev.off()
...
# top.features <- 
#  head(SpatiallyVariableFeatures(
...
SpatialFeaturePlot(
  brain, features = top.features, ncol = 3, alpha = c(0.1, 1))
...
# cells this speeds up SCTransform dramatically with no loss in performance
  ...
  SCTransform(
    allen_reference, ncells = 3000, verbose = FALSE) %>%
  RunPCA(verbose = FALSE) %>%
  RunUMAP(dims = 1:30) -> allen_reference

# After subsetting, we renormalize cortex
cortex <- 
  SCTransform(cortex, assay = "Spatial", verbose = FALSE) %>%
  RunPCA(verbose = FALSE)
...
top.clusters = rownames(
  dplyr::slice_min(
    cortex[["predictions"]]@meta.features,
    moransi.spatially.variable.rank,
    n = 4
  )
)
...
slide.seq@assays$Spatial@data@Dimnames[[1]] # These are Riken (?) transcript (?) identifiers
...
## This step will take ~ 1hr
SpatialFeaturePlot(slide.seq, features = top.features2, ncol = 3, alpha = c(0.1, 1), max.cutoff = "q95")
